...
const { substitute } = require('../webpack/dynamics');
...
class SzConfig {
    constructor(root, userOverrides = {}) {
        ...
        for (let varName in this.dynamics) {
            ...
            // prerender some data of dynamics that will be exposed later to pages
            descriptor.data = {
                entries: Object.entries(descriptor.values).map(
                    ([key, value]) => ({ key, value })
                ),
    ...
    // page data
    getDataForPage({ fullPageName, dynamics }) {
        fullPageName = substitute(fullPageName, dynamics);
        const pagePath = fullPageName.slice();
        ...
        // unwrap dynamics
        for (let varName in this.dynamics) {
            if (varName in dynamics) {
                // this dynamic has a value assigned
                result.dynamics[varName] = this.getDynamic(varName, dynamics[varName]);
            } else {
                // this dynamic is undefined here
                result.dynamics[varName] = { ...this.dynamics[varName].data };
            }
        }
