...
const { tree, rerequire } = require('../utils');
...
function loadComponent({
    ...
    if (fs.existsSync(classFilepath) && !fs.statSync(classFilepath).isDirectory()) {
        component = rerequire(classFilepath);
        if (!component.name) component.name = componentName;

        if (!(component instanceof SzComponent)) {
            // if object was exported
            component = new SzComponent(component);
        }
    } else {
